...
namespace ff {
  OpenGLFrameBuffer::OpenGLFrameBuffer(TextureBuffer::Format format, i32 width, i32 height, void* data) {
    ...
    bind();
    gl->FramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, textureBuffer_->handle(), 0);
    FF_ASSERT_E(gl->CheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE, "Error setting up frame buffer!");
    unbind();
  }
